...
chunksize <- 1000   # size of chunks to process (for large matrices)
...
flows <- read_csv("wu03ew_v1.csv") %>% 
    ...
    filter(weight >= 500,                # remove flows with weight < 20 and 
           o != d)                       # internal flows
...
knnm <- knn %>% 
    melt() %>% 
    acast(Var1 ~ value)
knnm[!is.na(knnm)] <- TRUE
knnm[is.na(knnm)] <- FALSE
## method B (doesn't work)
diss <- matrix(integer(0), nrow = 0, ncol = nrow(knn))
for(i in 0:round(nrow(knn)/chunksize)) {
    cat(i)
    diss <- rbind(diss, 
                  outer((i * chunksize) + 1:min(nrow(knn), (i + 1) * chunksize), 
                        1:nrow(knn), 
